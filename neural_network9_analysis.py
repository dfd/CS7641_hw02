...
def run_analysis():
    with open('./nn_output/neural_network9_nn_results_dict.pkl', 'rb') as f:
        results = pickle.load(f)

    #print(results)
    raw_data = results['synthetic']
    ...
    plt.savefig('./nn_plots/lr_final_total_fitness_curve.png')
    ...
    plt.savefig('./nn_plots/lr_final_zoom_fitness_curve.png')
    ...
    plt.suptitle('Fitness vs Time for each algorithm with Logistic Regression')
    plt.legend()
    plt.plot()
    plt.savefig('./nn_plots/lr_final_time_ba.png')
    #plt.show()
    ...
    plt.legend()
    plt.plot()
    plt.savefig('./nn_plots/lr_final_time_f1.png')
 
    weights_df = pd.DataFrame.from_dict(
        {(i, j, k): raw_data[i][j][k]['weights']
                    for i in raw_data.keys()
                    for j in raw_data[i].keys()
                    for k in raw_data[i][j].keys()
            }, orient='index'
    )
    weights_df.index = pd.MultiIndex.from_tuples(weights_df.index,
                                                names=('algorithm', 'hyperparameters', 'run'))
    
    print('weights_df')
    print(weights_df)
    weights_df.to_csv('./nn_output/lr_final_weights.csv')
